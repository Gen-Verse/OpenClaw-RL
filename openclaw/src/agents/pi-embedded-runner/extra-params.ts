...
function createSessionMetadataHeadersWrapper(
  baseStreamFn: StreamFn | undefined,
  sessionId: string,
  turnType: "main" | "side",
): StreamFn {
...
export function applyExtraParamsToAgent(
  ...
  thinkingLevel?: ThinkLevel,
  agentId?: string,
  sessionMetadata?: { sessionId: string; turnType: "main" | "side" },
): void {
  ...
  if (sessionMetadata) {
    agent.streamFn = createSessionMetadataHeadersWrapper(
      agent.streamFn,
      sessionMetadata.sessionId,
      sessionMetadata.turnType,
    );
  }
}
