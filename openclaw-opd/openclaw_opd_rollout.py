...
class AsyncRolloutWorker:
    def __init__(self, args, data_buffer):
        ...
        self._server = OpenClawOPDAPIServer(
            args=args,
...
async def _drain_output_queue(args, worker: AsyncRolloutWorker) -> list[list[Sample]]:
    ...
    # Strict behavior: only valid OPD samples are ever enqueued by server.
    # So we wait until we truly collect rollout_batch_size effective samples.
